...
export interface SamsungTizenDevice {
    serial: string;
    state: string;
}

export interface SamsungTizenAppInfo {
    id: string;
    name: string;
...
export class SamsungTizenService implements DeviceProvider {
    ...
    async connect(host: string, port = 26101): Promise<string> {
        ...
        return invoke<string>('plugin:samsung-tizen|tizen_connect', {serial});
    }
    ...
    async install(serial: string, filePath: string): Promise<string> {
        ...
        return invoke<string>('plugin:samsung-tizen|tizen_install', {serial: s, filePath});
    }
    ...
    async uninstall(serial: string, appId: string, runtimeId?: string | null): Promise<string> {
        const s = this.normalizeSerial(serial);
        return invoke<string>('plugin:samsung-tizen|tizen_uninstall', {serial: s, appId, runtimeId});
    }

    async getAppVersion(serial: string, appId: string): Promise<string> {
        const s = this.normalizeSerial(serial);
        return invoke<string>('plugin:samsung-tizen|tizen_get_app_version', {serial: s, appId})
            .catch(() => '');
    }

    async installViaTizenBrew(serial: string, filePath: string): Promise<string> {
        const s = this.normalizeSerial(serial);
        return invoke<string>('plugin:samsung-tizen|tizen_install_tizen_brew', {serial: s, filePath});
    }
    ...
    async getTizenBrewDeviceDetails(serial: string): Promise<{systemInfo: TizenInfoEntry[], daemonError: string}> {
        ...
        return invoke<{systemInfo: TizenInfoEntry[], daemonError: string}>(
            'plugin:samsung-tizen|tizen_tizen_brew_device_details', {serial: s}
        ).catch(() => ({systemInfo: [], daemonError: 'TizenBrew daemon not available'}));
    }

    async listApps(serial: string): Promise<PlatformApp[]> {
        const s = this.normalizeSerial(serial);
        const raw = await invoke<RawTizenAppInfo[]>('plugin:samsung-tizen|tizen_list_apps', {serial: s});
        return raw.map(a => ({
            id: a.id,
            name: a.name,
    ...
    async getDeviceInfo(serial: string): Promise<DeviceInfo> {
        ...
        return invoke<DeviceInfo>('plugin:samsung-tizen|tizen_get_device_info', {serial: s})
    ...
    async launch(serial: string, appId: string): Promise<string> {
        const s = this.normalizeSerial(serial);
        return invoke<string>('plugin:samsung-tizen|tizen_launch', {serial: s, appId});
    }

    async kill(serial: string, appId: string): Promise<string> {
        const s = this.normalizeSerial(serial);
        return invoke<string>('plugin:samsung-tizen|tizen_kill', {serial: s, appId});
    }

    async debug(serial: string, appId: string, runtimeId?: string | null): Promise<number> {
        const s = this.normalizeSerial(serial);
        return invoke<number>('plugin:samsung-tizen|tizen_debug', {serial: s, appId, runtimeId});
    }
    ...
    async getDeviceDuid(serial: string): Promise<string> {
        const s = this.normalizeSerial(serial);
        return invoke<string>('plugin:samsung-tizen|tizen_get_duid', {serial: s});
    }

    async shell(serial: string, command: string): Promise<string> {
        const s = this.normalizeSerial(serial);
        return invoke<string>('plugin:samsung-tizen|tizen_shell', {serial: s, command});
    }

    async tizenDaemonCommand(serial: string, command: string): Promise<string> {
        const s = this.normalizeSerial(serial);
        return invoke<string>('plugin:samsung-tizen|tizen_daemon_command', {serial: s, command});
    }
